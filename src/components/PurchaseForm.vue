  ...
  methods: {
    buyCripto() {
      if (this.sellBuy.crypto_amount === "") {
        ...
        this.$toast.error("You must enter the date and time of purchase.");
      } else {
        transactions
          .newTransaction(this.sellBuy)
          .then(() => {
            this.$toast.info("Purchase made successfully!");
          })
          .catch(() => {
            this.$toast.error("Error when making the purchase.");
          });
      }
    },
  },
